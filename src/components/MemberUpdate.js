...
function MemberUpdate() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:9070/member/${id}`)
      .then(res => {
  ...
  const handleRegionChange = ({ region, city, town }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const updateData = {
      ...
      phone: form.phone,
      email: form.email,
      region: form.region,  // 도/특별시/광역시
      city: form.city,      // 시/군/구
      town: form.town,      // 읍/면/동
    };
        ...
        <fieldset>
          ...
          <p>
            ...
            <input
              ...
              value={form.password}
              onChange={handleChange}
              placeholder="회원가입시 입력한 비밀번호 또는 변경할 비밀번호"
            />
          </p>
          <p>
          ...
          <p className="logform_initial_region">
            <label htmlFor="logform_init_region">
              입력하신 지역 <span>(지역 수정시 화면에 변경되어 나옵니다.)</span>
            </label>
            <input
              type="text"
              value={`${form.region} ${form.city} ${form.town}`.trim()}
              readOnly
          ...
          <p className="logform_cancel">
            <button type="button" value="취소" onClick={handleClick}>
              취소
            </button>
          </p>
        </fieldset>
